 ...
 *  - 方向键 ←/→（getKeys 轮询边沿检测）与 /tv next|prev 命令
...
/** 键盘切换检测（getKeys 轮询 100ms）：方向键在观战/副驾下都不触发
 *  onKeyStateChange（SA-MP 文档明确），统一由轮询读 getKeys().leftRight。
 *  Q/E 不做切换：观战模式下客户端把 Q/E 当本地镜头键不上传（实机验证
 *  onKeyStateChange 与 getKeys 位集都收不到），方向键是唯一可靠的按键入口。
 *  只响应观战者，按下瞬间（与上次读数变化）触发一次；spectate 模式客户端
 *  不发 sync 包，getKeys 可能返回陈旧/杂音值——边沿检测仅状态变化触发一次，
 *  FIRE 不绑定（瞄准/氮气键）。切换入口：方向键 ←/→ + /tv next|prev 命令。 */
...
export function stopObserve(
  ...
  observePrevLeftRight.delete(player.id); // 清轮询按键残留（防 playerId 复用时吞掉首次按键切换）
 ...
 * /tv next|prev；副驾只切车辆目标，见 pollObserveKeys/cycleObserveTarget）。
...
export function cleanupObserve(playerId: number): void {
  ...
  observePrevLeftRight.delete(playerId); // 清轮询按键残留（防 playerId 复用误切）
...
export function startObserveTicks(): void {
  ...
  // 不上传（实机验证 onKeyStateChange 与 getKeys 位集都收不到）。
  ...
  PlayerEvent.onCommandText(["tv", "ob", "spec"], ({ player, subcommand, next }) => {
    ...
    if (!arg) {
      sysMsg(
        player,
        "observe",
        "用法: /tv 玩家ID 观战 · /tv off 关闭 · /tv next|prev 切换（观战中方向键 ←/→ 也可切车）",
        "info",
      );
      return next();
    }
