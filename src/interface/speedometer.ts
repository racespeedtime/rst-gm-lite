 ...
 * 整体 y 比原版上移 20px：给底部调试信息栏（y=440 起）让位，避免刻度被叠
 */
export function createSpeed2d(player: Player): TextDraw[] {
  const tds: TextDraw[] = [];

  tds.push(
    new TextDraw({ player, x: 517.0, y: 402.5, text: "KM/H" })
      .create()
  ...
  tds.push(
    new TextDraw({ player, x: 516.0, y: 391.5, text: "-1" })
      .create()
      .setAlignment(3)
      .setBackgroundColors(255)
      .setFont(2)
  ...
  for (let i = 0; i < 20; i++) {
    tds.push(
      new TextDraw({ player, x: i * 4 + 455.0, y: 410.5, text: "/" })
        .create()
        .setBackgroundColors(255)
        .setFont(1)
