...
- **氮气补给（单管 15s 现实寿命模型）**：一罐氮气对**现实时间**只能撑 15s（实机确认）。补管间隔（播放时间）= `NITRO_AUTO_MS(15000) × speed`——任何倍速下现实恒定 15s，罐子刚好在耗尽前补上（0.25x→3750ms 播放=15s 现实；1x→15s；4x→60s 播放=15s 现实），慢放断罐从根上消失。喷氮气由录制帧 keys 驱动（录制者按 FIRE/ACTION → 包带位 → 客户端喷），补罐只保证"罐满"，补罐帧顺带强制 FIRE|ACTION 一次（兜底补罐点恰好落在松键瞬间）。challenge 影子固定 1x（=15s 播放）同模型。
